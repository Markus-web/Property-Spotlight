...
**Note:** These are Linear.fi's public test API credentials, shared for development and testing purposes. They connect to a sandbox environment with sample property data. See [API documentation](https://support.linear.fi/hc/en-fi/articles/31404123207069-API-Integration-Listing-Data) for details (including Swagger for the test host).

**If listings never load with the test URL:** Confirm the host still matches Linear’s article (they may change it without notice). From any machine that should mirror your server’s connectivity, you can check for HTTP 200:

```bash
curl -sS -o /dev/null -w "%{http_code}\n" \
  -H "Accept: application/json" \
  -H "Authorization: LINEAR-API-KEY b6b23692-e3fa-4525-9709-e021c8a65baa" \
  "https://ca-externalapi-test-weu-001.livelyrock-4a193af6.westeurope.azurecontainerapps.io/v2/listings?languages[]=fi"
```

If this hangs, times out, or returns an error, the sandbox is unreachable from that network—contact [Linear support](https://support.linear.fi/) for the current test endpoint. Production integrations should use your brokerage’s own Data URL and key.
...
5. If errors mention timeout or cURL error 28, verify HTTPS access to your Data URL from the server (firewall, DNS, or Linear’s test host may be down—see **Test API** above and [Linear’s API article](https://support.linear.fi/hc/en-fi/articles/31404123207069-API-Integration-Listing-Data))
6. HTTP 502/503/504 from the API usually means a temporary problem on Linear’s side; retry later or contact Linear support
