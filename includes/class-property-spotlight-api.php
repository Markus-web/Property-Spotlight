...
class Property_Spotlight_API {
    ...
    public function get_all_listings(string $lang = 'fi'): array|\WP_Error {
        ...
        if ($status_code !== 200) {
            ...
            $msg = sprintf(__('API returned status %d', 'property-spotlight'), $status_code);
            if (in_array($status_code, [502, 503, 504], true)) {
                $msg .= ' ' . __(
                    'The API server may be temporarily unavailable; try again later or contact Linear support.',
                    'property-spotlight'
                );
            }
